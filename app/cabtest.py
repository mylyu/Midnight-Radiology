...
with sync_playwright() as pw:
    ...
    # 县医院开场
    pg.goto(URL); pg.evaluate(f"localStorage.setItem('{KEY}', {json.dumps(mk('n1_s0',1))})")
    ...
    print('县人民医院:', '县人民医院' in pg.locator('body').inner_text())
    # 增感屏配图
    pg.goto(URL); pg.evaluate(f"localStorage.setItem('{KEY}', {json.dumps(mk('n3_cab1',3))})")
    pg.reload(); pg.wait_for_timeout(600)
    bt=pg.locator('button:has-text("继续")')
    if bt.count(): bt.first.click(); pg.wait_for_timeout(500)
    b2=pg.locator('button:has-text("回到夜班现场"), button:has-text("出发，上夜班")')
    ...
    print('item_screen 渲染:', 'item_screen.png' in imgs)
    pg.screenshot(path='/tmp/t_screen.png')
