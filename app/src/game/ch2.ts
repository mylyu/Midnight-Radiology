...
export function ch2StepForState(id: string, step: Step, state: Pick<GameState, 'flags' | 'badges' | 'gender' | 'finished'> & Partial<Pick<GameState,'ap'|'items'>>): Step {
  ...
  if (id === 'c2d2_16a' && flags.remote_asked) {
    text = flags.lei_cable
      ? '小凯说回头给字段清单，小雷说那条线单走外网。两句话放在一起，还缺了中间那一截。「我去问清楚。你那份申请也别停。」'
      : '小凯答应过要字段清单，到现在还没见着。「回头我去追一下。你别拿厂家的做法当批件，申请照走。」'
  }
      ...
      : flags.data_audit
        ...
        : flags.data_oppose
          ...
          : flags.data_support
            ...
            : '主任在交接记录上签了字。'
    text = '周一早上八点，办公室先过昨夜交班：CT仍停机，工程师负责复核；远程终端保持离线。信息科今天接手查日志，周五反馈。' + reply + '老周把排班表推过来，你的名字在主值栏，他的在备班栏。'
  }
  return text === step.text && queue === step.queue ? step : { ...step, text, queue }
}
...
export const CH2_EVENTS: Record<string, ChronicleEvent> = {
  ...
  ch2_luzhou: { time: '2025年11月', title: '陆舟来院', body: '本科室友陆舟跟导师做低剂量重建科研，来院完成体模实验，留下记录。临床数据申请还在走流程，双方没有私下拷走患者数据。' },
...
const C2N1: Record<string, Step> = {
  ...
  c2n1_4: { speaker: 'zhou', sprite: 'char_zhou', sfx: 'vox_ch2_natural_zhou', text: "有事叫我。（老周站在操作台旁，朝你点点头）我今晚也在。", next: 'c2n1_5' },
  ...
  // —— B. 小凯交底 ——
  c2n1_b1: { speaker: 'kai', sprite: 'char_kai', sfx: 'vox_ch2_natural_kai', text: "有事打我电话。（小凯指了指墙上的表）日检和预热的步骤都贴这儿了。报警看不明白，拍给我，别反复点确认。", card: 'ct_tube_heat', effect: { ap: -1 }, next: 'c2n1_b2' },
  ...
  // —— E. 走廊转转 · 小雷 ——
  c2n1_e1: { speaker: 'lei', sprite: 'char_lei', sfx: 'vox_ch2_natural_lei', text: "图传过去了。PACS这边也通了，今晚直接在工作站看。（小雷松开鼠标，甩了甩手）我去把线号贴完。", effect: { ap: -1 }, next: 'c2n1_e2' },
  ...
  c2n1_c4a: { speaker: 'tang', sprite: 'char_tang', text: "可不！它吃电，咱吃剩饭。主任倒是提了，照这个量，明年可能申请第二台。我先问能不能多配两个人，护士长让我别做梦。", next: 'c2n1_c5' },
  ...
  c2n1_c4c: { speaker: 'tang', sprite: 'char_tang', text: "（接过咖啡罐）欸，下午老周在机房里站了半天。我问他是不是舍不得老机器，他问我：夜里停机，病人往哪儿送？……你说这人，聊天都能聊出张排班表。", next: 'c2n1_c5' },
  ...
  c2n1_m1: { speaker: 'he', sprite: 'char_he', sfx: 'vox_ch2_natural_he', text: "麻烦让一下！（小何跟着平车进门）养老院送来的，起夜摔下床，头磕在床头柜上。现在叫不太醒，右侧瞳孔有点大。急诊申请了头颅CT。", next: 'c2n1_m2' },
  ...
  c2n1_m7: { bg: 'bg_ctcontrol', speaker: 'sys', text: '定位像 → 设定范围 → 扫描。「嗡——」十几秒，第一幅横断图像跳上屏幕。', sfx: 'xray', next: 'c2n1_m8' },
  ...
  c2n1_p_scan: { bg: 'bg_ctcontrol', speaker: 'sys', text: "小伙子躺稳后，检查床缓缓进到机架里。采集结束，进度条转到了重建。", sfx: 'xray', next: 'c2n1_p3' },
...
const C2D2: Record<string, Step> = {
  ...
  c2d2_1: { speaker: 'director', sprite: 'char_director', sfx: 'vox_ch2_natural_director', text: "今天人不少啊。（主任朝候诊区看了一眼）队列你先接着，急诊来了叫我。", effect: { flag: 'day_shift' }, queue: C2D2_QUEUE0, next: 'c2d2_reg0' },
  ...
  c2d2_lung_scan: { bg: 'bg_ctcontrol_day', speaker: 'sys', text: "大爷按提示屏住气。扫描完成，工作站收到胸部数据，你先打开了默认的厚层序列。", sfx: 'xray', next: 'c2d2_4' },
  ...
  c2d2_7: { speaker: 'uncle', sprite: 'pat_uncle2', sfx: 'vox_ch2_natural_uncle', text: "哦，这回看清了。（大爷凑近薄层重建的图，手指悬在屏幕前）去年说看不见，我还当它没了。", effect: { gold: 80 }, image: 'ct_lung', next: 'c2d2_8' },
...
const C2N3: Record<string, Step> = {
  ...
  // —— A. 雯雯 · 维保合同 ——
  c2n3_a1: { bg: 'bg_corridor', speaker: 'wen', sprite: 'char_wen', sfx: 'vox_ch2_natural_wen', text: "还没下班啊？（雯雯把合同夹进包里）我跟你们主任磨了一下午维保。第一年免费，后面全保、半保、技保，差着好几万，他一条一条问。", effect: { ap: -1 }, next: 'c2n3_a2' },
  ...
  c2n3_m5: { bg: 'bg_ctcontrol', speaker: 'me', text: '开机、定位像、扫描——「嗡——」', sfx: 'xray', dnt: 24, next: 'c2n3_m6' },
  ...
  c2n3_repeat_scan: { bg: 'bg_ctcontrol', speaker: 'sys', text: "头托与固定垫调整好，人员退出机房。受影响范围补扫完成，新的序列传到了工作站。", sfx: 'xray', next: 'c2n3_m8' },
  ...
  c2n3_cta_scan: { bg: 'bg_ctcontrol', speaker: 'sys', text: "对比剂按方案团注，确认动脉强化后完成CTA采集，工作站开始重建血管图像。", sfx: 'xray', next: 'c2n3_m10' },
  ...
  c2n3_mystery_scan: { bg: 'bg_ctcontrol', speaker: 'sys', text: "老人慢慢躺下，双手交叠在腹前。检查床驶过机架；采集结束，控制台开始重建。", sfx: 'xray', next: 'c2n3_x8' },
...
const C2D4: Record<string, Step> = {
  ...
  c2d4_e8: { speaker: 'sys', text: "主任把核查单压在彩页上，拨通厂家的电话：「别再给我讲免费，先把谁负责写清楚。」陆舟的消息同时弹来：「材料退回来了，少一页导师签字。」你回：别省这页，明天一起问。", event: 'ch2_data_showdown', effect: { flag: 'data_hook' }, next: 'c2d4_reg' },
...
const C2N5: Record<string, Step> = {
  ...
  c2n5_m7a: { speaker: 'duty', phone: 'char_duty', sfx: 'vox_ch2_natural_duty', text: "先把旧片发来。今晚新出现的症状也说一下，我跟急诊一起看，再定要不要扫。", next: 'c2n5_m8' },
...
const C2AM: Record<string, Step> = {
  ...
  c2am_1: { speaker: 'director', sprite: 'char_director', sfx: 'vox_ch2_natural_director_am', text: "来，聊几个问题。（主任把椅子往桌边拉了拉）五道，老规矩。说完就散，别耽误大家吃早饭。", next: 'c2am_2' },
