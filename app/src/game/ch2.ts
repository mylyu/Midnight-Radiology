...
export const CH2_CARDS: Record<string, KnowledgeCard> = {
  ...
  contrast_checklist: {
    title: '增强前核对清单',
    body: '过敏史、肾功能、甲状腺功能、用药（二甲双胍）、禁食水、知情同意——六格清单每一格都对应一种真实发生过的事故。打药前花两分钟逐项核对，是影像科最便宜的安全装置。',
    image: 'ct_liver_plain',
  },
  contrast_contra: {
    title: '增强禁忌：eGFR/甲亢/二甲双胍',
    body: 'eGFR低于45，对比剂肾病风险陡增；二甲双胍遇上对比剂可能诱发乳酸酸中毒；甲亢未控制，碘负荷可能诱发甲状腺危象。禁忌清单不是吓人的，是用病例换来的——稳妥了再做，不差这两天。',
    image: 'ct_abdomen',
  },
  contrast_emergency: {
    title: '过敏反应分级处置',
    body: '轻度（荨麻疹、瘙痒）：停药、观察、抗组胺；中度：加用激素；重度（喉头水肿、休克）：肾上腺素、气道管理、呼叫抢救。分级的意义是既不缺位也不过度——上来就肾上腺素，是把轻症往心律失常里推。',
    image: 'ct_liver_enhanced',
  },
  dose_ct: {
    title: 'CT剂量：看得见的账',
...
export const CH2_BOOK_PAGES: { title: string; body: string; note: string }[] = [
  ...
  {
    title: '第12页 · 碘对比剂——先过禁忌清单',
    body: '非离子型碘剂（优维显、欧乃派克）温和但不是零风险。增强前六件事：过敏史、肾功能（eGFR低慎做）、甲状腺（甲亢未控暂缓）、二甲双胍（遇上对比剂警惕乳酸酸中毒）、禁食水、知情同意。清单一格格过，不凭印象。',
    note: '页边钢笔字：「清单短，事故长。」',
  },
  {
    title: '第13页 · 过敏了怎么办——分级处置',
    body: '轻度：皮疹、瘙痒——停药、观察、抗组胺。中度：加用激素、吸氧、监测。重度：喉头水肿、休克——肾上腺素、气道管理、呼叫抢救。轻症不会等你慢慢升级成重症，但一上来就推肾上腺素，是把轻症往心律失常里推。分级，然后果断。',
    note: '页边钢笔字：「不慌，不等于不动。」',
  },
  {
...
const C2D4: Record<string, Step> = {
  c2d4_0: { bg: 'bg_office_day', speaker: 'sys', text: '周五白班。主任把你叫到办公室。', next: 'c2d4_1' },
  c2d4_1: { speaker: 'director', sprite: 'char_director', text: '今天增强专场。**增强扫描是CT的另一半江山**，也是风险最高的一环。今天你的任务：把增强流程从里到外吃透，零差错。', next: 'c2d4_2' },
  c2d4_2: { speaker: 'sys', text: '【增强前核对清单】每位增强病人进门，六格清单逐项核对：过敏史、肾功能、甲状腺功能、用药、禁食水、知情同意。漏一项，风险就找上门。', card: 'contrast_checklist', next: 'c2d4_3' },
  c2d4_3: { bg: 'bg_ctcontrol_day', speaker: 'sys', text: '第一例：肝占位，增强三期扫描。', checklist: { items: [
    { label: '过敏史', value: '无碘对比剂过敏史' },
    { label: '肾功能', value: 'eGFR 88' },
    { label: '甲状腺功能', value: '正常' },
    { label: '用药', value: '无二甲双胍' },
    { label: '禁食水', value: '已禁食 6 小时' },
    { label: '知情同意', value: '已签字' },
  ], next: 'c2d4_4' }, next: 'c2d4_4' },
  c2d4_4: { speaker: 'me', text: '（核对清单）过敏史无、eGFR 88、甲亢无、无二甲双胍、禁食6小时、同意书已签——**碘对比剂，上！**', next: 'c2d4_5' },
  c2d4_5: { speaker: 'zhou', sprite: 'char_zhou', text: '（在旁讲解）**碘，原子序数53，高密度、强吸收，打进血管里，血管和富血供病灶瞬间「亮」起来**。现在用的都是**非离子型碘剂，优维显、欧乃派克这一类**，比老式离子型温和得多——但温和不等于零风险，所以才有门口那份清单。', image: 'ct_liver_plain', card: 'contrast_agent', next: 'c2d4_6' },
  c2d4_6: { speaker: 'zhou', sprite: 'char_zhou', text: '为什么要打三期？**药推进去，先进动脉——动脉期；再过四十秒上下，血流经门静脉灌进肝——门脉期；再往后，药慢慢从病灶里退出来——延迟期**。肝癌是「快进快出」——动脉期一下子亮起来，门脉期就淡了；血管瘤正相反，「快进慢出」，行话叫「早出晚归」——**动脉期先从边缘一圈结节样亮起来，然后一帧一帧向中心填充，到延迟期还滞留着不走**。时间本身就是诊断。', image: 'ct_liver_hemangioma', next: 'c2d4_7' },
  c2d4_7: { speaker: 'enh', sprite: 'pat_enh', sfx: 'vox_enh', text: '医生，这药打进去热乎乎的，一股热流从脸上烫到尾椎骨——像喝了口白酒！', image: 'ct_liver_enhanced', next: 'c2d4_8' },
  c2d4_8: { speaker: 'sys', text: '三期图像行云流水，病灶边界清清楚楚。', image: 'ct_liver_enhanced', next: 'c2d4_t1' },
  // —— 期相识别小考 ——
  c2d4_t1: { speaker: 'zhou', sprite: 'char_zhou', text: '（调出刚才的三期图像，把期相标注一遮）趁热打铁，考考你——**这一幅，病灶边缘一圈结节样亮起来，肝实质还没怎么亮。是哪一期？**', image: 'ct_liver_hemangioma', choices: [
    { text: '「动脉期——药刚进动脉，病灶边缘先亮，肝实质还没灌到。」', next: 'c2d4_t1ok', effect: { skill: 1, flag: 'phase_t1' }, tag: 'good' },
    { text: '「门脉期——药从门静脉灌进肝了。」', next: 'c2d4_t1no' },
    { text: '「延迟期——药已经开始从病灶里退了。」', next: 'c2d4_t1no' },
  ]},
  c2d4_t1ok: { speaker: 'zhou', sprite: 'char_zhou', text: '对。**动脉期看动脉和富血供病灶的边缘，肝实质主要靠门静脉供血，这会儿还没轮上它亮**——边缘结节样强化，正是血管瘤「早出晚归」的「早」。', image: 'ct_liver_hemangioma', next: 'c2d4_t2' },
  c2d4_t1no: { speaker: 'zhou', sprite: 'char_zhou', text: '再看一眼：**肝实质还没亮，说明门静脉那波药还没到——这是动脉期**。血管瘤的戏，就从边缘这一圈结节样强化开场。', image: 'ct_liver_hemangioma', next: 'c2d4_t2' },
  c2d4_t2: { speaker: 'zhou', sprite: 'char_zhou', text: '再考一个——这幅：**肝实质整体亮起来了，血管和病灶的对比最分明**。哪一期？', image: 'ct_liver_enhanced', choices: [
    { text: '「门脉期——药经门静脉灌进肝，肝实质强化到顶峰。」', next: 'c2d4_t2ok', effect: { skill: 1 }, tag: 'good' },
    { text: '「动脉期。」', next: 'c2d4_t2no' },
    { text: '「延迟期。」', next: 'c2d4_t2no' },
  ]},
  c2d4_t2ok: { speaker: 'zhou', sprite: 'char_zhou', text: '**门脉期是肝脏增强的主战场**——大部分病灶靠门脉期定性质。行，眼睛练出来了。', image: 'ct_liver_enhanced', next: 'c2d4_t3ok' },
  c2d4_t2no: { speaker: 'zhou', sprite: 'char_zhou', text: '肝实质整体发亮，是门静脉供血到顶峰的标志——**门脉期**。记住这个手感，以后扫肝脏，门脉期是必答题。', image: 'ct_liver_enhanced', next: 'c2d4_t3ok' },
  c2d4_t3ok: { speaker: 'sys', text: '两幅期相，一眼一个。老周难得地挑了下眉毛，在你的带教本上写了四个字：「**期相过关**」。', skipUnlessFlag: 'phase_t1', effect: { badge: 'phase_eye' }, next: 'c2d4_9' },
  c2d4_9: { speaker: 'me', text: '（下一例：腹痛待查，老年男性，糖尿病十年。核对清单——**eGFR 42，二甲双胍每日口服**）……停。eGFR低于45，**对比剂肾病风险陡增**；而且**二甲双胍遇上对比剂，乳酸酸中毒能要命**。', card: 'contrast_contra', checklist: { items: [
    { label: '过敏史', value: '无碘对比剂过敏史' },
    { label: '肾功能', value: 'eGFR 42', alert: true },
    { label: '甲状腺功能', value: '正常' },
    { label: '用药', value: '二甲双胍 · 每日口服', alert: true },
    { label: '禁食水', value: '已禁食 5 小时' },
    { label: '知情同意', value: '已签字' },
  ], next: 'c2d4_10' }, next: 'c2d4_10' },
  c2d4_10: { speaker: 'sys', text: '【家属在门外探头：「大夫，我们请假来的，今天能做上吗？」——怎么办？】', choices: [
    { text: '「暂停增强：请内分泌会诊调药、停二甲双胍、充分水化，复查eGFR达标后改期再做。」', next: 'c2d4_11a', effect: { skill: 2, heart: 1 }, tag: 'good' },
    { text: '「家属催得紧——先做平扫把今天对付过去，增强以后再说。」', next: 'c2d4_11b', effect: { skill: -1, flag: 'c2n4_wrong' } },
    { text: '「对比剂剂量减半，同时开静脉水化，边做边观察。」', next: 'c2d4_11c', effect: { skill: -1 } },
  ]},
  c2d4_11a: { speaker: 'me', text: '（对大爷解释）您这个肾功能，打药伤肾；二甲双胍和对比剂碰上，可能出大事。咱先停药、补水、复查肾功能，**稳妥了再做，不差这两天**。', next: 'c2d4_12a' },
  c2d4_12a: { speaker: 'grandpa', sprite: 'pat_grandpa2', sfx: 'vox_grandpa', text: '你们这医院，真较真！……行，听你们的，命要紧。', next: 'c2d4_13' },
  c2d4_11b: { speaker: 'zhou', sprite: 'char_zhou', text: '对付？**腹痛待查的老人家，平扫看不清肠道血供，漏了缺血就是漏了命**——该做的检查，是把风险降下来再做，不是糊弄过去。去，跟大爷一家把话说清楚：先调药，改期做增强。', next: 'c2d4_13' },
  c2d4_11c: { speaker: 'zhou', sprite: 'char_zhou', text: '**减量不减风险**——禁忌的门槛不是剂量堆出来的，eGFR 42就是42。水化帮的是「可以做的人」，不能把「不能做」洗成「能做」。去，跟病人好好解释，改期。', next: 'c2d4_13' },
  c2d4_13: { speaker: 'sys', text: '第三例：甲状腺超声报「结节4a类」，来做颈部增强。', next: 'c2d4_14' },
  c2d4_14: { speaker: 'me', text: '（核对清单——**TSH 0.01，亚临床甲亢**）……甲亢未控制，**碘对比剂可能诱发甲状腺危象**。**禁忌清单第三条：甲亢未控，增强暂缓**。', checklist: { items: [
    { label: '过敏史', value: '无碘对比剂过敏史' },
    { label: '肾功能', value: 'eGFR 76' },
    { label: '甲状腺功能', value: 'TSH 0.01 · 亚临床甲亢', alert: true },
    { label: '用药', value: '无相关用药' },
    { label: '禁食水', value: '已禁食 6 小时' },
    { label: '知情同意', value: '已签字' },
  ], next: 'c2d4_15' }, next: 'c2d4_15' },
  c2d4_15: { speaker: 'sys', text: '一上午核对清单六格全绿、零漏项。', effect: { badge: 'checklist_zero' }, next: 'c2d4_p0' },
  // —— 下午 · 过敏反应应急 ——
  c2d4_p0: { speaker: 'sys', text: '下午第一例，乳腺肿块增强。对比剂刚推进去——', sfx: 'buzz', next: 'c2d4_p1' },
  c2d4_p1: { speaker: 'sys', text: '【病人突然喊：「**痒！我脖子痒！**」——荨麻疹以肉眼可见的速度爬上脖颈。应急处置？】', choices: [
    { text: '立即停药，吸氧，肌注苯海拉明，监测生命体征——轻度过敏反应，按流程处理。', next: 'c2d4_p2a', effect: { skill: 2, badge: 'allergy_save' }, tag: 'good' },
    { text: '「观察观察，说不定自己就好了。」', next: 'c2d4_p2b', effect: { skill: -1, flag: 'c2n4_risk' } },
    { text: '「快！肾上腺素！」', next: 'c2d4_p2c', effect: { skill: -1 } },
  ]},
  c2d4_p2a: { speaker: 'me', text: '**轻度过敏：停药、观察、抗组胺；中度：加用激素；重度：肾上腺素、气道管理、呼叫抢救**——分级处理，不慌不乱。', card: 'contrast_emergency', next: 'c2d4_p3a' },
  c2d4_p3a: { speaker: 'sys', text: '十分钟后，皮疹消退，病人生命体征平稳。你按流程把处置经过写进护理记录，又交代病人留观半小时——下一次再有病人喊「痒」，你的手不会再抖了。', next: 'c2d4_e0' },
  c2d4_p2b: { speaker: 'zhou', sprite: 'char_zhou', text: '**观察就是赌命**。轻度过敏可能在十分钟内升级为喉头水肿——**该出手时不出手，是对「狠」最大的误解**。立刻按流程处理！', card: 'contrast_emergency', next: 'c2d4_p3b' },
  c2d4_p2c: { speaker: 'zhou', sprite: 'char_zhou', text: '**这是轻度，不是重度**——上来就肾上腺素，是把病人往心律失常里推。**分级！分级！分级！**先停药、吸氧、抗组胺！', card: 'contrast_emergency', next: 'c2d4_p3b' },
  c2d4_p3b: { speaker: 'sys', text: '你按流程走完处置，十分钟后皮疹消退。老周没再说什么，但那个眼神你记住了。', next: 'c2d4_e0' },
  // —— 傍晚 · 数据回传摊牌 ——
