...
assert.equal(report.count, 197); assert.equal(report.optimized, 194)
assert.equal(report.images.length, 197)
assert.equal(report.deliveredBytes, reviewed.stats.deliveredImageBytes)
...
console.log('PASS delivery compatibility: historical corrupt-cache recovery retained; live prepare report/catalog and all 197 real delivery hashes, byte counts and dimensions agree; numerical inputs exact; newly approved encoding policy audited separately')
