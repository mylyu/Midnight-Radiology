...
assert.equal(steps.c2n5_p2d, undefined, 'Deferred classmate messages must not play')
...
assert.doesNotMatch(steps.c2n5_g0.text, /机器仍停着|分流/)
assert.match(steps.c2am_0.text, /CT正常交接/)
assert.match(steps.c2am_0.text, /信息科今天接手查日志，周五反馈/)
assert.equal(steps.c2n5_r0, undefined, 'Water-ring scene retired')
...
for (const id of ['c2n1_m3a', 'c2n1_m3b', 'c2d2_3', 'c2d2_11', 'c2n5_m7b', 'c2n5_5']) assert.equal(steps[id].image, undefined, id + ' must not reveal future result')
