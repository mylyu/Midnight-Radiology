...
try {
  ...
  for (const viewport of [{ width: 844, height: 390 }, { width: 740, height: 320 }]) {
    await page.setViewportSize(viewport)
    await boot(fixture(4, {}, 3))
    await page.getByRole('button', { name: /去旧片库看看/ }).waitFor()
    const box = await page.locator('.dialog-box').boundingBox()
    const toolbar = await page.getByRole('button', { name: '📖 手册', exact: true }).boundingBox()
    assert(box.y >= toolbar.y + toolbar.height, `Landscape dialog covers toolbar: ${JSON.stringify({ box, toolbar })}`)
    await choose(/去旧片库看看/, 'n4_arc0')
    await next('n4_arc1')
    await choose('算了，不属于自己的地方少待', 'n4_hub')
    await page.getByRole('button', { name: /去检查室开工/ }).scrollIntoViewIfNeeded()
    await page.screenshot({ path: `C:/Users/lvmen/AppData/Local/Temp/archive-landscape-${viewport.height}.png` })
    const root = await page.locator('[data-ch1-step]').boundingBox()
    assert.equal(root.x, 0); assert.equal(root.y, 0)
  }
