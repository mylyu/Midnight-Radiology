...
const rows=JSON.parse(readFileSync(new URL('../../docs/ch2-natural-voices-lines.json',import.meta.url),'utf8')).lines
const url=process.env.GAME_URL||'http://127.0.0.1:8798/'
const out=process.env.VOICE_OUTPUT||'../../ch2-natural-voices-review'
...
try{
 for(const row of rows){
  ...
  await context.addInitScript(({shift,row})=>{
   ...
   localStorage.setItem('midnight-radiology-save-v1',JSON.stringify({gender:'m',night:5,gold:500,skill:3,wealth:3,heart:3,durability:70,badges:[],stamps:[],flags:{['heard2_vp3_'+row.old_id]:true},lastCheckin:'',streak:0,finished:true,seed:1234,items:[],ap:3,buyCount:0,cards:[],events:[],dlc:{ch2:{shift,stepId:row.step,viewBg:'bg_ctcontrol'}}}))
  },{shift:shift.id,row})
  const page=await context.newPage()
  page.on('pageerror',e=>errors.push(e.message))
  await page.goto(url+'#/ch2')
  await page.locator('[data-ch2-step="'+row.step+'"]').waitFor()
  await page.waitForFunction(id=>window.__ended.some(src=>src.includes('/'+id+'.mp3')),row.id,{timeout:15000})
  await page.waitForFunction(text=>document.querySelector('.dialog-box > p')?.textContent?.startsWith(text),row.text)
  const calls=await page.evaluate(()=>window.__played.filter(r=>r.src.includes('/vox')))
  assert.equal(calls.length,1,row.id+' played more than once')
  assert(calls[0].src.includes(row.id+'.mp3'))
  assert.equal(calls[0].volume,.45)
  assert(calls[0].duration>.5&&calls[0].duration<4)
  assert.equal(await page.getByRole('button',{name:/显示全文|继续 →/}).count(),0)
  results.push({step:row.step,...calls[0],ended:true})
  await context.close()
 }
 ...
 assert.equal(await page.locator('audio').count(),28)
 assert.equal(await page.evaluate(()=>localStorage.length),0)
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true)
 await page.getByRole('button',{name:'连听新配音',exact:true}).click()
 ...
 console.log('PASS: 14 actual Edge audio decodes/plays/ends, exact new clip and unchanged speech gain; preview playlist/stop/mobile/no-save checks.')
