...
assert.equal(steps.c2n1_b1.sfx, 'vox_ch2_natural_kai_noref_b_v9')
...
assert.match(steps.c2n3_h4.text, /薄层断层图.*重组图.*逐段核对/)
assert.equal(steps.c2n3_h4.image, undefined, 'Reviewed CTA revision keeps clinical recheck text without replaying the slice popup')
assert.equal(steps.c2n3_h5.image, undefined)
assert.match(steps.c2d4_aorta_resist.text, /跑半马.*踢球.*背上的筋抽了.*止个疼/)
assert.equal(steps.c2d4_aorta_doctor.speaker, 'he')
assert.match(steps.c2d4_aorta_doctor.text, /突然就疼得很厉害.*一直不缓解.*不能当普通拉伤/)
...
for (const id of ['c2d4_aorta_resist', 'c2d4_aorta_wife', 'c2d4_aorta_doctor', 'c2d4_aorta_consent']) {
  const s = render(id)
  assert([s.sprite, s.sprite2].includes('ch2_patient_aorta_middle_bed'), `${id}: reviewed middle-aged patient disappeared during resistance`)
}
