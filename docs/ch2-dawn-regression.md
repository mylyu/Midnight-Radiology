...
发布 `dc0ba43` 后线上定向测试通过，但第二次人工复看发现667×375横屏名牌上沿被原`overflow-y:auto`裁剪：原检查只覆盖正文和选项，漏掉名牌。补上整块名牌在对话框边界内的断言；新场景专属CSS在矮屏把名牌排入框内，不改公共对话框或第一章。修补后另跑针对测试/保护检查/构建；不把此前四轮完整流程说成针对这条CSS又重跑四遍。原线上记录在仓库旁 `ch2-dawn-live-review/`，修后截图与报告单独保存。

修补后开发服与8801生产预览的完整日出针对测试均通过，`errors=[]`，人工复看确认名牌、正文和两选项完整显示；报告位于 `ch2-dawn-badge-review/`、`ch2-dawn-badge-production-review/`。构建、定向lint和实时冻结检查重新通过。
